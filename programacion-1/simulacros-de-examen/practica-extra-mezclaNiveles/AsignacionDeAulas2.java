public class AsignacionDeAulas2 {
/*
...
*/
    ...
    public static void main(String[] args) {
        ...
        procesarSecuencia(materias, aulas);

        mostrarArreglo(materias);
    }

    public static void procesarSecuencia(int[] materias, int[] aulas){
        int ini = 0;
        int fin = -1;
        int contadorDeMaterias = 0;

        while(ini < materias.length){
            ini = buscarInicio(materias, fin + 1);

            if(ini < materias.length){
                fin = buscarFin(materias, ini);

                int tamaño = fin - ini + 1;
                contadorDeMaterias++;

                int aulaParaMaterias = buscarAulaLibre(aulas, tamaño);

                if(aulaParaMaterias != -1){

                    aulas[aulaParaMaterias] = -aulas[aulaParaMaterias];

                    for(int i = 0; i < tamaño; i++){
                        corrimientoIzquierda(materias, ini);
                    }
                    fin = fin - tamaño;

                    System.out.println("la materia: " + contadorDeMaterias + " se asigno al aula: " + aulaParaMaterias);
                }
                else{
                    System.out.println("la materia: " + contadorDeMaterias + " no se puede asignar.");
                }
            }
        }
    }

    public static int buscarAulaLibre(int[] aulas, int tamaño){
        int pos = 0;
        boolean aulaEncontrada = false;

        while(pos < aulas.length && !aulaEncontrada){
            if(aulas[pos] >= tamaño && aulas[pos] > -1){
                aulaEncontrada = true;
            }
            else{ 
                pos++; 
